...
// Тёмный туман с почти чёрным фоном
static const char* FS = "precision mediump float; uniform vec4 c; uniform vec3 l; uniform int mode; varying vec3 v_n; varying float v_d; void main(){ if(mode==2){gl_FragColor=c; return;} float df=max(dot(v_n,l),0.15); vec4 res=vec4(c.rgb*df,c.a); float fog=clamp((v_d-3.0)/12.0,0.0,1.0); gl_FragColor=mix(res,vec4(0.03,0.03,0.05,1.0),fog); }";
