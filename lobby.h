...
// ИСПРАВЛЕНО: Добавлен параметр use_tex_loc
static inline void draw_lobby(GLint mvp_loc, GLint col_loc, GLint use_tex_loc, GLuint p_loc, GLuint uv_loc, int w, int h, Font* font, mat4 view) {
    // Фон
    glUniform1i(use_tex_loc, 0); // Рисуем цветом
    ...
    // Текст "PLAY"
    glUniform1i(use_tex_loc, 2); // Включаем режим рендеринга текста
    glUniform4f(col_loc, 1.0f, 1.0f, 1.0f, 1.0f); // Белый цвет
    glBindTexture(GL_TEXTURE_2D, font->tex_id);
    
    float text_x = w/2.0f - 60.0f; // Центрируем текст вручную
    float text_y = h/2.0f + 15.0f;
