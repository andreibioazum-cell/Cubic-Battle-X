...
static void draw(struct engine* eng) {
    ...
    // Логика веревки
    if(g->r_act == 1) { // Летит
        g->r_x += sinf(g->ry)*cosf(g->rx)*0.5f; g->r_z += cosf(g->ry)*cosf(g->rx)*0.5f; g->r_y += sinf(g->rx)*0.5f;
        if(sqrtf(pow(g->r_x-g->x,2)+pow(g->r_z-g->z,2)) > 10.0f) g->r_act = 0;
    }
    ...
    mat4_mul(&mv, &pv, &mo); draw_box(eng, &mv, 0.2, 0.2, 0.2, 1, 0);

    // Руки (Квадраты)
    glDisable(GL_DEPTH_TEST);
    mat4_id(&mo); mo.m[12]=0.6; mo.m[13]=-0.5; mo.m[14]=-1.0; mo.m[0]=0.3; mo.m[5]=0.3; mo.m[10]=0.3;
    mat4_mul(&mv, &pr, &mo); draw_box(eng, &mv, 0.2, 0.2, 0.2, 1, 0);
    mo.m[12]=-0.6; mat4_mul(&mv, &pr, &mo); draw_box(eng, &mv, 0.2, 0.2, 0.2, 1, 0);

    // Веревка и Синяя перчатка
    if(g->r_act) {
    ...
    if(g->tj) {
        ...
        mat4_mul(&mv, &or, &mo); draw_box(eng, &mv, 0, 0, 0, 0.5, 2); // Кольцо (черное)
        mat4_id(&mo); mo.m[12]=g->jcx; mo.m[13]=g->jcy; mo.m[0]=60; mo.m[5]=60;
        mat4_mul(&mv, &or, &mo); draw_box(eng, &mv, 1, 1, 1, 1, 2); // Стик (белый)
    }
    // Кнопка броска
    mat4_id(&mo); mo.m[12]=w-150; mo.m[13]=h-150; mo.m[0]=120; mo.m[5]=120;
    mat4_mul(&mv, &or, &mo); draw_box(eng, &mv, 0, 0.5, 1, 0.6, 2);
    
    draw_text(eng, 60, w-150, 50, &or);
...
static int32_t handle_input(struct android_app* app, AInputEvent* ev) {
    ...
    if (a == AMOTION_EVENT_ACTION_DOWN || a == AMOTION_EVENT_ACTION_POINTER_DOWN) {
        if (x > w-250 && y > h-250) { g->r_act=1; g->r_x=g->x; g->r_y=1.6; g->r_z=g->z; }
        ...
        for(int p=0; p<AMotionEvent_getPointerCount(ev); p++) {
            ...
            if(px<w/2) { float dx=px-g->jsx, dy=py-g->jsy, d=sqrtf(dx*dx+dy*dy); if(d>70){dx*=70/d;dy*=70/d;} g->jcx=g->jsx+dx; g->jcy=g->jsy+dy; }
