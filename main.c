...
static void draw_frame(struct engine* eng) {
    ...
    if(eng->state == STATE_LOBBY) {
        ...
        // ДЖОЙСТИК С ОБВОДКОЙ И ПРОЗРАЧНОСТЬЮ
        glUniform1i(eng->use_tex_loc, 0); 
        // 1. Обводка базы (Черная)
        glUniform4f(eng->col_loc, 0, 0, 0, 1.0f);
        ui_draw_circle(eng->mvp_loc, eng->p_loc, eng->joy.sx, eng->joy.sy, 105, view);
        // 2. Фон базы (Прозрачный серый)
        glUniform4f(eng->col_loc, 0.5f, 0.5f, 0.5f, 0.2f);
        ui_draw_circle(eng->mvp_loc, eng->p_loc, eng->joy.sx, eng->joy.sy, 100, view);
        // 3. Пипка (Черная)
        glUniform4f(eng->col_loc, 0, 0, 0, 1.0f);
        ui_draw_circle(eng->mvp_loc, eng->p_loc, eng->joy.cx, eng->joy.cy, 45, view);
    }
