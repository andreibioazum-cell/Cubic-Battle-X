...
void draw_box(struct engine* eng, mat4* mvp, float r, float g, float b, float a, int mode) {
    glUniformMatrix4fv(eng->mvp_loc, 1, 0, mvp->m); glUniform4f(eng->col_loc, r, g, b, a); glUniform1i(eng->mode_loc, mode);
...
static void draw(struct engine* eng) {
    ...
    // FPS расчет (вывод в лог)
    struct timespec now; clock_gettime(CLOCK_MONOTONIC, &now);
    long t = now.tv_sec * 1000 + now.tv_nsec / 1000000;
    if(t - eng->last_f >= 1000) { eng->last_f = t; eng->fps = 0; } eng->fps++;

    if (eng->tj) {
        float dx=(eng->jcx-eng->jsx)/100.0f, dz=(eng->jcy-eng->jsy)/100.0f;
        float s=sinf(eng->ry), c=cosf(eng->ry);
        eng->x += (dx*c-dz*s)*0.15f; eng->z += (dx*s+dz*c)*0.15f;
    }

    glViewport(0, 0, w, h); glClearColor(0.05f, 0.05f, 0.05f, 1.0f); glClear(GL_COLOR_BUFFER_BIT|GL_DEPTH_BUFFER_BIT);
    ...
    for(int i=0; i<10; i++) for(int j=0; j<10; j++) {
        ...
        mat4_id(&mo); mo.m[12]=i*3.0f; mo.m[14]=j*3.0f; mo.m[5]=6.0f; mo.m[0]=3.0f; mo.m[10]=3.0f;
        mat4_mul(&mv, &pv, &mo); 
        if(MAP[i][j]==1) draw_box(eng, &mv, 0.2f, 0.2f, 0.2f, 1.0f, 0);
        else draw_box(eng, &mv, 0.8f, 0.1f, 0.1f, 1.0f, 0); // ЦЕЛЬ
    }
    mat4_id(&mo); mo.m[13]=0; mo.m[0]=100.0f; mo.m[10]=100.0f; mo.m[5]=0.01f;
    mat4_mul(&mv, &pv, &mo); draw_box(eng, &mv, 0.1f, 0.12f, 0.1f, 1.0f, 0);

    // ДЖОЙСТИК (Белое кольцо, черный стик)
    ...
    if (eng->tj) {
        ...
        mat4_mul(&mv, &or, &mo); draw_box(eng, &mv, 0, 0, 0, 0.7f, 2);
    }
    // FPS ИНДИКАТОР (Квадратик в углу: зеленый - плавно, красный - лагает)
    mat4_id(&mo); mo.m[12]=w-50; mo.m[13]=50; mo.m[0]=40; mo.m[5]=40;
    mat4_mul(&mv, &or, &mo); draw_box(eng, &mv, (eng->fps < 30 ? 1.0f : 0.0f), (eng->fps >= 30 ? 1.0f : 0.0f), 0, 1, 2);
...
static int32_t handle_input(struct android_app* app, AInputEvent* ev) {
    ...
    if (a == AMOTION_EVENT_ACTION_DOWN || a == AMOTION_EVENT_ACTION_POINTER_DOWN) {
        ...
        for(int p=0; p<AMotionEvent_getPointerCount(ev); p++) {
            ...
            if(px<w/2) { float dx=px-eng->jsx, dy=py-eng->jsy, d=sqrtf(dx*dx+dy*dy); if(d>80) {dx*=80/d; dy*=80/d;} eng->jcx=eng->jsx+dx; eng->jcy=eng->jsy+dy; }
