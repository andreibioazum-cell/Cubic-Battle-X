...
// ИСПРАВЛЕНО: Добавлено сглаживание (мипмапы) и размытие
GLuint load_tex(struct engine* eng, const char* name) {
    ...
    // Сглаживание для песка (убирает шум)
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
    // Размытие для игрока (убирает пикселизацию)
...
static void draw(struct engine* eng) {
    ...
    // ИСПРАВЛЕНО: Джойстик ниже и левее
    ...
    if(eng->state == STATE_LOBBY) {
        ...
        // ИСПРАВЛЕНО: Огромный пол, чтобы не было черноты
...
static void handle_cmd(struct android_app* app, int32_t cmd) {
    ...
    if(cmd == APP_CMD_INIT_WINDOW) {
        // ... инициализация EGL ...
        // ИСПРАВЛЕНО: Создаем карту при старте
        create_map_borders();
