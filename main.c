...
static void draw(struct engine* eng) {
    ...
    eng->joy.sx = 180; eng->joy.sy = h - 180; // Фикс позиции в углу
    ...
    if(eng->state == STATE_LOBBY) {
        ...
        mat4 world = view; mat4_translate(&world, w/2.0f - eng->player.x, h/2.0f - eng->player.y);
        glUniform1i(eng->use_tex_loc, 1);
        glBindTexture(GL_TEXTURE_2D, eng->floor_tex); draw_quad(eng->mvp_loc, -4000, -4000, 8000, 8000, 80, 80, world);
        glBindTexture(GL_TEXTURE_2D, eng->player_tex); draw_quad_ext(eng->mvp_loc, eng->player.x, eng->player.y, 110, 110, 1, 1, eng->player.angle, world);
        glUniform1i(eng->use_tex_loc, 0); glUniform4f(eng->col_loc, 0,0,0,1);
        ui_draw_circle(eng->mvp_loc, eng->joy.sx, eng->joy.sy, 90, 4, view); // Обводка 4 (меньше)
    ...
    if (AInputEvent_getType(ev) == AINPUT_EVENT_TYPE_MOTION) {
        ...
        if(eng->state == STATE_LOBBY) {
            ...
            if(code == AMOTION_EVENT_ACTION_DOWN || code == AMOTION_EVENT_ACTION_POINTER_DOWN) {
                ...
                if(id == eng->joy.joy_pid) { eng->joy.active=0; eng->joy.joy_pid=-1; }
            }
        }
        return 1;
    }
...
static void handle_cmd(struct android_app* app, int32_t cmd) {
    ...
    if(cmd == APP_CMD_INIT_WINDOW) {
        ...
        eng->state = STATE_LOBBY; eng->joy.joy_pid = -1; eng->player.speed = 12.0f;
